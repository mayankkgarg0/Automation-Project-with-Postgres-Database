...
public class RejectedAndReopenedServiceRequestScenarioTest extends BaseClass{
	...
	private static Logger logger =Logger.getLogger(RejectedAndReopenedServiceRequestScenarioTest.class);
	
	@BeforeClass
	public void getDBConnection(){
		logger.info("In BeforeClass method of "+RejectedAndReopenedServiceRequestScenarioTest.class.getSimpleName());
	...
	@Test(priority=23, description="Rejected And Reopened service request")
	public void verifyRejectedServiceRequestTest() throws SQLException, InterruptedException, TimeoutException{
		int patientNum = 2;
		ROLES patientRole = Constants.ROLES.PATIENT;
		String patient = patientRole.name().toLowerCase();
		String testName = "verifyRejectedAndReopenedServiceRequestTest";
		...
		if (expectedTitle.equals(actualTitle)){
			...
			logger.info(message);
			extentTest.log(LogStatus.FAIL, "verifyRejectedAndReopenedServiceRequestTest method is failed !!");
			...
			/*--------------Reject Service Request process [placed at 1st row in table]----------------------------*/
			driver.navigate().to(EMPLOYEE_BASE_URL+"/super/assigned");
			driver.manage().timeouts().pageLoadTimeout(Constants.PAGE_LOAD_TIMEOUT, TimeUnit.SECONDS);
			driver.manage().timeouts().pageLoadTimeout(Constants.IMPLICIT_WAIT, TimeUnit.SECONDS);
			logger.info("Navigated to Assigned Service Requests section");
			Thread.sleep(2000);
			empDash.rejectServiceRequest();	
			logger.info("Service request rejecetd !!");
			driver.navigate().to(EMPLOYEE_BASE_URL+"/super/rejected");
			driver.manage().timeouts().pageLoadTimeout(Constants.PAGE_LOAD_TIMEOUT, TimeUnit.SECONDS);
			driver.manage().timeouts().pageLoadTimeout(Constants.IMPLICIT_WAIT, TimeUnit.SECONDS);
			wait.until(ExpectedConditions.urlContains("/super/rejected"));
			logger.info("Navigated to Rejected Service Requests screen");
			Thread.sleep(2000);	
			...
			//Switch to patient window to verify the status of service request
			...
			wait.until(ExpectedConditions.urlContains(PATIENT_BASE_URL+"/user/service/dashboard"));
			Thread.sleep(2000);
			String expectedStatus = Constants.REQUEST_STATUS.Rejected.name();
			...
			logger.info("Status of "+expectedTitle+" is updated as: "+actualstatus);
			Thread.sleep(2000);
			
			/*--------------Reopen Service Request [placed at 1st row in table]----------------------------*/
			//switch to employee window to reopen rejected service request
			...
			wait.until(ExpectedConditions.urlContains("/super/rejected"));
			Thread.sleep(2000);
			logger.info("Switched to employee screen to reopen rejected request !!");
			empDash.reopenServiceRequest();	
			logger.info("Rejected SR reopened by employee");
			driver.navigate().to(EMPLOYEE_BASE_URL+"/super/new");
			driver.manage().timeouts().pageLoadTimeout(Constants.PAGE_LOAD_TIMEOUT, TimeUnit.SECONDS);
			driver.manage().timeouts().pageLoadTimeout(Constants.IMPLICIT_WAIT, TimeUnit.SECONDS);
			wait.until(ExpectedConditions.urlContains("/super/new"));
			Thread.sleep(2000);
			logger.info("Navigated to New Service Requests screen to view reopened request");
			String expectedStatusReopened = Constants.REQUEST_STATUS.Reopened.name();
			status = driver.findElement(By.xpath("//table[@class='table-box']/tbody/tr["+row+"]/td[3]/tc-badge"));
			actualstatus = status.getText();
			Assert.assertEquals(actualstatus,expectedStatusReopened, expectedStatusReopened+" status not found for "+expectedTitle+" on employee dashboard");
			logger.info("Status of "+expectedTitle+" is updated as: "+actualstatus);
			...
			status = driver.findElement(By.xpath("//table[@class='table-box']/tbody/tr["+row+"]/td[3]/tc-badge"));
			actualstatus = status.getText();
			Assert.assertEquals(actualstatus,expectedStatusReopened, expectedStatusReopened+" status not found for "+expectedTitle+" on patient dashboard");
